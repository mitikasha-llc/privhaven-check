  ...
  if (located.length === 0) {
    ...
    if (table) {
      for (const f of located) {
        ...
        if (validator === null) { add('soundness', `Finding: ${label}`, 'info', `Type "${f.type}" is header-led (validator "${f.validator}") and is not value-checkable from the file alone. Column present with ${cells.length} non-empty cell(s).`); continue; }
        ...
        if (countOk && validatedOk) add('soundness', `Finding: ${label}`, 'pass', `${cells.length} non-empty cell(s); ${validated} pass ${f.validator}, matching the report.`);
